CREATE OR REPLACE FUNCTION get_supplier_products_by_vehicle(
  p_supplier_id UUID,
...
BEGIN
  ...
  LEFT JOIN supplier_product_vehicles spv ON spv.supplier_product_id = p.id
  LEFT JOIN vehicles v ON v.id = spv.vehicle_id
