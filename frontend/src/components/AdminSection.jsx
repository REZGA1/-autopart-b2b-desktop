...
import { getUnvalidatedSuppliers, validateSupplier } from '@/lib/merchantApi';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { Button } from '@/components/ui/button';

// Accordion Supplier Card - Compact view with expandable details
...
// Inline document thumbnail that opens in fullscreen
...
export default function AdminSection({ onValidateSuccess }) {
  ...
  // Poll for new suppliers every 5 seconds (background update)
  useEffect(() => {
  ...
  const handleValidate = async (supplierId) => {
    try {
      ...
      // Remove the validated supplier from the list
      setSuppliers(prev => prev.filter(s => s.id !== supplierId));
      // Notify parent that validation occurred (so ContactsSection can refresh)
