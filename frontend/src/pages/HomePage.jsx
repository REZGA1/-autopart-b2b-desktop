...
export default function HomePage() {
  ...
  // Trigger refetch for ContactsSection when validation occurs
  const [contactsRefetchTrigger, setContactsRefetchTrigger] = useState(0)

  const handleSupplierValidated = () => {
    // Increment trigger to cause ContactsSection to refetch
  ...
  return (
    <Layout title="Dashboard">
      <div className="space-y-6">
        ...
        {/* Administration section - only for merchants */}
        {profile?.role === 'merchant' && (
          <AdminSection onValidateSuccess={handleSupplierValidated} />
        )}

        {/* Contacts section - for both merchants and suppliers */}
        <ContactsSection 
          role={profile?.role} 
